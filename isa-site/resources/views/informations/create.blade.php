...
@section('content')
<h1>Adding a new information</h1>
<form action="{{ route('informations.store', ['information' => $information]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-6">
            <select name="department_id" class="form-control" id="department_id" required>
                ...
                <option value="{{ $id }}" {{ (isset($information->department_id) && $id === $information->department_id) ? 'selected' : '' }}>{{ $display }}</option>
                @endforeach
            </select>
        </div>
    </div>
