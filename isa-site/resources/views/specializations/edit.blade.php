...
@section('content')
<h1>Editing {{$specialization->name}}</h1>
<form action="{{ route('specializations.update', ['specialization' => $specialization]) }}" method="POST" enctype="multipart/form-data">
    @method('PUT')
    @csrf
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-6">
            <select name="department_id" class="form-control" id="department_id" required>
                ...
                <option value="{{ $id }}" {{ (isset($department) && $id === $department->id) ? 'selected' : '' }}>{{ $display }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-6">
            <select name="user_id" class="form-control" id="user_id" required>
                ...
                <option value="{{ $id }}" {{ (isset($responsible) && $id === $responsible->id) ? 'selected' : '' }}>{{ $display }}</option>
                @endforeach
            </select>
        </div>
    </div>
